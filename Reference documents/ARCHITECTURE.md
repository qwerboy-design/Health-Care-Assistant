...
### 7. UI 佈局系統 (`components/chat/` - v1.2.3)

|| 元件 | 功能 |
||------|------|
|| `ChatWindow.tsx` | 動態佈局容器，根據對話輪次調整比例 |
|| `ChatInput.tsx` | 自適應輸入框，支援動態高度調整 |
|| `MessageList.tsx` | 訊息列表，配合父層 flex 控制 |

**設計決策**：
- **動態佈局邏輯**：根據 `Math.floor(messages.length / 2)` 計算對話輪次
  - ≤2 輪（0-4 則訊息）：空白狀態，輸入框優先（flex-[3] vs flex-[2]）
  - >2 輪（5+ 則訊息）：活躍狀態，訊息區優先（flex-1 vs 固定高度）
- **自適應輸入框**：
  - 空白狀態：3 行初始高度，max-h-[300px]，引導使用者輸入
  - 活躍狀態：1 行初始高度，max-h-[150px]，節省空間
- **平滑過渡**：300ms CSS transition，確保狀態切換不突兀
- **測試覆蓋**：42 個測試用例（ChatWindow: 17, ChatInput: 25），100% 通過率
- **UX 優化**：
  - 新用戶引導：大輸入框鼓勵開始對話
  - 閱讀體驗：對話開始後，更多空間展示 AI 回覆
  - 響應式設計：維持原有斷點，不影響移動端體驗
